...
3. When the connection opens, the transport fires `onReady`, so `bot.on('ready', ...)`
...
log) when there is no connected socket (for example mid-reconnect), so a typing indicator
...
- **Buttons**: Baileys has no native quick-reply buttons, so the transport renders a
  **numbered text fallback** appended to the text:
  ...
  The user answers with plain text (the number or the title), so `buttonId` never
